...
import { PlantingCeremony } from '../../../components/PlantingCeremony';
import { justPlanted } from '../../../lib/treeAwards';
...
export default function CompleteScreen() {
  ...
  // A plan finished on the exact count a tree is gated behind just planted it.
  // Derived, never stored: nothing marks a tree as newly unlocked, so nothing
  // can go stale or fire twice for one planting.
  const arrived = finishedPlans === null ? null : justPlanted(finishedPlans);

  // One success beat for the finish, and the planting owns it when a tree
  // arrives. Firing at mount as well would sound it twice a breath apart.
  useEffect(() => {
    if (counted && !arrived) haptics.success();
  }, [counted]);
  ...
  // The grove grows with plans finished, not days read. This is the one screen
  // where it means something: it moved here because on Today it sat beside the
  // streak bar saying the same thing twice.
  useEffect(() => {
    if (!couple?.id) { setCounted(true); return; }
    finishedPlanCount(couple.id)
      .then(setFinishedPlans)
      .catch(() => {})
      // Settled either way. A count we could not read plants nothing and takes
      // the ordinary haptic, rather than leaving the finish silent.
      .finally(() => setCounted(true));
  }, [couple?.id]);
  ...
  return (
    // The planting is a sibling of the safe area, not a child of it, so it can
    // cover the whole display and apply the insets itself.
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <Text variant="body" color={colors.ink2} style={styles.subtitle}>
              Hey, you did amazing. You showed up, you did the work, and you grew.
            </Text>
          </View>
          ...
          {finishedPlans !== null && (
            <GroveCard
              count={finishedPlans}
              onPress={() => router.push('/(tabs)/you/grove', { withAnchor: true })}
            />
          )}
      ...
      {/* The planting covers this screen while it plays, and leaves it standing
          underneath. Both ways out land somewhere: the Grove, or here. */}
      {arrived && planted && (
        <PlantingCeremony
          award={arrived}
          onSeeGrove={() => {
            setPlanted(false);
            router.push(
              { pathname: '/(tabs)/you/grove', params: { arrived: arrived.id } },
              { withAnchor: true },
            );
          }}
          onDismiss={() => setPlanted(false)}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  statsCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginBottom: 10 },
