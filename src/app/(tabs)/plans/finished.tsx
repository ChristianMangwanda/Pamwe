...
export default function FinishedPlansScreen() {
  ...
  const [plans, setPlans] = useState<FinishedPlan[] | null>(null);
  ...
  return (
    <Screen>
      ...
      <Text variant="h1" style={styles.title}>Finished together</Text>
      <Text variant="journal" italic color={colors.ink2} style={styles.subtitle}>
        Every plan you read to the end.
      </Text>
      ...
      {plans === null ? (
        ...
        <Text style={[styles.empty, { color: colors.muted }]}>
          The first plan you finish together shows up here.
        </Text>
      ) : (
          ...
          {plans.map((p) => (
            <TouchableOpacity key={p.couplePlanId} activeOpacity={0.85} onPress={() => open(p.planId)}
              ...
              <View style={[styles.rowIcon, { backgroundColor: colors.surface2, borderColor: colors.lineAccent }]}>
                <CheckCircle size={19} color={colors.accent2} weight="fill" />
              </View>
...
function metaFor(p: FinishedPlan): string {
  const days = `${p.durationDays} days`;
