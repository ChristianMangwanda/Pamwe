#!/usr/bin/env python3
"""Fetch World English Bible text for all M'Cheyne plan passages via bible-api.com.

Handles single chapters (Genesis 1), multi-chapters (Genesis 9-10),
verse ranges (Luke 1:1-38), and Psalm ranges (Psalms 119:1-24).

Rate limit: 15 requests per 30 seconds. We sleep 2.5s between requests.
"""
...
def ref_to_api_url(ref: str) -> str:
    """Convert a passage reference to a bible-api.com URL.

    Handles:
    - 'Genesis 1' -> genesis+1
    - 'Genesis 9-10' -> genesis+9:1-10:32 (multi-chapter)
    - 'Luke 1:1-38' -> luke+1:1-38 (verse range)
    - 'Psalms 119:1-24' -> psalms+119:1-24
    - 'Psalms 42-43' -> psalms+42:1-43:5
    - 'Psalms 86-87' -> psalms+86:1-87:7
    """
    import re

    # Already has verse numbers (e.g., "Luke 1:1-38", "Psalms 119:1-24")
    if ":" in ref:
        return "https://bible-api.com/" + ref.lower().replace(" ", "+") + "?translation=web"

    # Multi-chapter range (e.g., "Genesis 9-10", "1 Samuel 5-6")
    m = re.match(r'^(.+?)\s+(\d+)-(\d+)$', ref)
    if m:
        book = m.group(1)
        ch_start = int(m.group(2))
        ch_end = int(m.group(3))
        last_verse = LAST_VERSES.get(book, {}).get(ch_end, 50)
        api_ref = f"{book} {ch_start}:1-{ch_end}:{last_verse}"
        return "https://bible-api.com/" + api_ref.lower().replace(" ", "+") + "?translation=web"

    # Single chapter (e.g., "Genesis 1")
    return "https://bible-api.com/" + ref.lower().replace(" ", "+") + "?translation=web"


def fetch_passage(url: str) -> dict:
    """Fetch a passage from bible-api.com."""
    req = urllib.request.Request(url, headers={"User-Agent": "Pamwe-App/1.0"})
    try:
...
def main():
    with open("scripts/mcheyne_plan.json") as f:
        plan = json.load(f)

    output_path = "scripts/mcheyne_with_text.json"
    ...
    done_days = {d["day_number"] for d in existing}
    results = list(existing)

    remaining = [d for d in plan if d["day_number"] not in done_days]
    total = len(plan)

    print(f"Already fetched: {len(done_days)}/{total}", file=sys.stderr)
    print(f"Remaining: {len(remaining)}", file=sys.stderr)

    for i, day in enumerate(remaining):
        ref = day["passage_reference"]
        url = ref_to_api_url(ref)

        print(f"[{len(done_days) + i + 1}/{total}] Day {day['day_number']}: {ref}", file=sys.stderr, end=" ")

        data = fetch_passage(url)

        if "error" in data and "verses" not in data:
            ...
            text = data.get("text", "")
            print(f"OK ({len(verses)} verses, {len(text)} chars)", file=sys.stderr)

            results.append({
                **day,
                "passage_text": text.strip(),
                "verse_count": len(verses),
                "verses": [{"chapter": v["chapter"], "verse": v["verse"], "text": v["text"].strip()} for v in verses],
            })

        # Save progress after every fetch
        results.sort(key=lambda x: x["day_number"])
        with open(output_path, "w") as f:
            json.dump(results, f, indent=2)

        # Rate limit: ~15 req / 30s => sleep 2.1s
        if i < len(remaining) - 1:
            time.sleep(2.1)

    print(f"\nDone! {len(results)} days saved to {output_path}", file=sys.stderr)
    ...
    if errors:
        ...
        for e in errors:
            print(f"  Day {e['day_number']}: {e['passage_reference']} - {e['error']}", file=sys.stderr)
