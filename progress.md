...
**▶ ROUND 3 IN FLIGHT: [`build8-plan.md`](build8-plan.md)** — Build 7 ramblings fully triaged and root-caused (headline: an infinite push-token loop was hammering PATCH /users at ~200 req/s, the source of most slowness + likely the crash). **All 9 fixes IMPLEMENTED (2026-07-10; tsc clean, 66/66 Jest) — see the "Implementation record" in build8-plan.md.** Sentry checked via the `sentry` CLI (the MCP never authed): **both b7 crashes attributed to the PATCH storm** — OOM stacks with 100× `PATCH /users` breadcrumbs (issues PAMWE-IOS-1/2 in org `zakia-12`; leave unresolved until b8 verifies). **b8 SHIPPED: uploaded to TestFlight 2026-07-10 20:51 PT** (awaiting Apple processing); ask-pamwe v6 (trimmed) live on hosted. Remaining: on-device pass on b8 (docked bar feel, cold-launch caches, Ask Pamwe timing), then resolve the two Sentry issues; watch for the b6 recaps crash. Note: `ios/ExportOptions.plist` was gitignore-lost and recreated; now un-ignored so it can't vanish again. Christian's rule: fix all of it before any green-list features. Remaining pre-b8 triage:
